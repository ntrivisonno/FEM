...
%
m_LongDiv = l./m_DivProb; % long del dominio
...
%
figure(1) % Figura Soluci'on exacta ->  %print -dpng Figs/e10_solu_P1.png
hold on
for iPr = 1:nProb
   if grPol==1
      ...
      %subplot(2,1,1);plot(m_ResPr(:,1),m_ResPr(:,2)) 
   end
end
...
xlabel ('x');
grid on
t1=sprintf('Solucion EDO - e = %d - Polinomio P_%d(x)\n',ee,grPol);
title(t1)
legend(s_NroElem,'uExac')
print(fullfile('Figs',[t1,'.png']),'-dpng')
   

figure(2)  % Figura derivada 1ra -> %print -dpng Figs/e10_deriv_solu_P1.png
hold on
for iPr = 1:nProb   
   if grPol==1
...
xlabel ('x');
grid on
t2=sprintf('Derivada 1ra de la EDO - e = %d - Polinomio P_%d(x)\n',ee,grPol);
title(t2)
legend(s_NroElem,'duExac')
print(fullfile('Figs',[t2,'.png']),'-dpng')


figure(3)  % Figura convergencia error -> %print -dpng Figs/e10_converg_error_P1.png
loglog(m_LongDiv,[m_ErrorL2,m_ErrorL2Der])
grid on
xlabel('Tama�o de los intervalos')
ylabel('Error')
t3=sprintf('Convergencia Error - e = %d - Polinomio P_%d(x)\n',ee,grPol);
title(t3)
legend({'u','du/dx'})
print(fullfile('Figs',[t3,'.png']),'-dpng')

figure(4) % Figura Error -> seguir
hold on
for iPr = 1:nProb
...
ylabel('Error')
legend(s_NroElem)
t4=sprintf('Error - e = %d - Polinomio P_%d(x)\n',ee,grPol);
title(t4)
print(fullfile('Figs',[t4,'.png']),'-dpng')


figure(5) % curva orden del error, no se entiende
hold on
for iPr = 1:nProb
   plot(m_Pend,'o-','MarkerFaceColor','b') %m_Pend = Pendiente num�rica de logaritmo del error
end
hold off
grid on
ylabel('Orden del error')
t5=sprintf('Orden de Error - epsilon = %d - Polinomio P_%d(x)\n',ee,grPol);
title(t5)
print(fullfile('Figs',[t5,'.png']),'-dpng')
