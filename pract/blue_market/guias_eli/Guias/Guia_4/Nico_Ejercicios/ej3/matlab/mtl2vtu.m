function mtl2vtu(FILE,NOD,ELE,SOL)
  ...
  %
  % modo de uso:
  % SOL = struct('Name','U','Type','PointData','NumberOfComponents',2,'Value',u')  % u debe ser vector colum
  ...
  switch(ETYPE)
    ...
    case {'QUAD16'}
      ...
      % No funciona. Paraview 3.12.0 no acepta cuadr�ngulos bi-c�bicos.
      % Para visualizaci�n, cada cuadr�ngulo bi-c�bico es dividido en
      % 9 cuadr�ngulos bi-lineales.
      etype = 9; % VTK_QUAD
      C = CONEC;
      CONEC = [C(:,[1 5 13 12])
               C(:,[5 6 14 13])
