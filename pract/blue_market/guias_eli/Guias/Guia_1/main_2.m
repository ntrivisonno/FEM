...
%Datos
%m_DivProb = [4,8,16,32,64,128,256,512,1024,2048,4096];      %Par para e=0.01
%m_DivProb=  [5,9,17,33,65,129,257,512,1025,2049,4097];   %Impar  para e=0.01
%m_DivProb = [4,8,16,32,64,128,256,512,1024,2048,4096];
%m_DivProb = [4,8,16,32,64,128,256,512,1024];  %Par para e=10
m_DivProb = [5,9,17,33,65,129,257,513,1025]  %Impar para e=10
...
for iPr = 1:nProb
   ...
   switch grPol
      case 1
         [m_ui,m_dui] = SolucionElemLineal_2(nElem,m_dom,s_f);         
      case 2
         m_ui = SolucionElemCuadr(nElem,m_dom,s_f);
         m_dui = zeros(size(m_ui));
      otherwise
         error('Solución de elemento finito no implementado.')
   end
   m_CoordNod = m_dom(1):l/nDiv:m_dom(2);
   [errorL2i,errorL2Deri,m_Error,c_Res] = f_ErrorL2Sol1D_2(f_uFNum,f_duFNum,m_ui,m_CoordNod,grPol);
   ...
   if grPol==1
      ...
      %En el caso del polinomios cuadráticos (o mayor) ya no se devuelve un valor de la derivada por
...
%% Pendiente numérica de logaritmo del error
...
hold off
ylabel ('Valor de du(x)/dx');
xlabel ('Posicion x ');
grid on
legend(s_NroElem)
saveas(gcf,'Derivada de u(x).png')
...
ylabel('Error')
legend({'u','du/dx'})
saveas(gcf,'Error en funcion del tamaño de los intevalos.png')
...
ylabel('Error')
legend(s_NroElem)
saveas(gcf,'Error en funcion de la posicion de X.png')
