function pltmsh4rotu(in,xx,iel,conec,u,locel,fac)
% function pltmsh4rotu(in,xx,iel,conec,u,locel,fac)
%
%   Dibuja una malla de elementos finitos cuadrangulares
%
%   in:    Numeros de nodos
...
%
    ...
    for i=1:nel
        plot( [X1(i,1) X1(i,2) X1(i,3) X1(i,4) X1(i,1)],        ...
              [Y1(i,1) Y1(i,2) Y1(i,3) Y1(i,4) Y1(i,1)], '-b')
    end

    if(length(in)<150)
        for j=1:nel
            numel=num2str(j);        
            text([(X1(j,1)+X1(j,2)+X1(j,3)+X1(j,4))/4],[(Y1(j,1)+Y1(j,2)+Y1(j,3)+Y1(j,4))/4],numel);
            for i=1:4
