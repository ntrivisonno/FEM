function [row,col,sk] = stiffcurejer5 (in,xx,iel,conec,locel,inn,indof,inel,kt,rega)
%
...
in1 = 0;
%%%Zona con conductividad kx=1 y ky=2
for ele = 1:nel;
       if ele ~= rega
                ...
                for i=1:3;
                    %Aca le agrego la conductvidad siendo kt(1)=kx y kt(2)=ky
                    beta(i)  =   (sqrt(kt(1,1))*(Y1( ind(i+1) ) - Y1( ind(i+2) )))/dosdelta;
                    gamma(i) =  (sqrt(kt(1,2))*(-(X1( ind(i+1) ) - X1( ind(i+2) ))))/dosdelta;
                 end
            ...
            for i=1:3;
                %Aca le agrego la conductvidad siendo kt(1)=kx y kt(2)=ky
                beta(i)  =   (sqrt(kt(2,1))*(Y1( ind(i+1) ) - Y1( ind(i+2) )))/dosdelta;
                gamma(i) =  (sqrt(kt(2,2))*(-(X1( ind(i+1) ) - X1( ind(i+2) ))))/dosdelta;
             end
