...
%  t: espesor (si t=0: tension plana, t>0: deformacion plana) 
...
for ele = 1:nel
    ...
    if t>0  % deformacion plana -> cuando no existen deformaciones en Z pero si tensiones (producto de coef Poisson).ej: tunel, dique.        % En dirección Z no puede haber deformación, tampoco deformaciones angulares gamma xz ni  yz.  
       C = E/(1-nu^2)*t* ...             
                         [1 nu 0         
                          nu 1 0
                          0 0 (1-nu)/2];
    else   %  tension plana -> no existen tensiones  pero si deformaciones en Z.  ej: placa delgada, recipiente a presion
        C = E/(1+nu)/(1-2*nu)* ...       
                         [1-nu nu 0       
                          nu 1-nu 0
                          0 0 (1-2*nu)/2];
