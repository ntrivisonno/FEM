function [row,col,sk] = stiffcurStokes (in,xx,iel,conec,locel,mu,inn,indof,inel)
nel = length (iel);
...
for ele = 1:nel
    ...
    %%%Escalado del elemento P5
    %Coodernadas del centro de gravedad
    xc = (x1+x2+x3)/3;
    yc = (y1+y2+y3)/3;
    % Parametro de escala L
    L = (1/2)*max([((x2-x1)^2+(y2-y1)^2)^(1/2),((x3-x2)^2+(y3-y2)^2)^(1/2),((x1-x3)^2+(y1-y3)^2)^(1/2)]);
        x1 = (x1-xc)/L;
        x2 = (x2-xc)/L;
        x3 = (x3-xc)/L;
        y1 = (y1-yc)/L;
    ...
    % Matriz de escala C
    Y = L^2;
    V=[1 L L Y Y Y 1 L L Y Y Y 1 L L Y Y Y];
    C=diag(V) ;
    ...
    %K = mu*B'*P*B;
      ...
      % Matriz de rigidez para elemento escalado
    K = mu*C*B'*P*B*C;
